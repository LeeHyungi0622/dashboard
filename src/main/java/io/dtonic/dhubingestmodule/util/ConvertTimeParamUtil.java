...
public class ConvertTimeParamUtil {
  ...
  public static String dateTimeToLocalDateTime(String strDateTime) {
    ...
    Pattern offsetDateTimePattern = Pattern.compile(
    ...
    try {
      if (iso8601Pattern.matcher(strDateTime).find()) {
        ...
        throw new BadRequestException(
          DataCoreUiCode.ErrorCode.REQUEST_MESSAGE_PARSING_ERROR,
    ...
    SimpleDateFormat transFormat = new SimpleDateFormat(
      Constants.POSTGRES_DATE_FORMAT
    );
   ...
   * @param endTime
   */
  public static void checkTimeRelParams(
    String timerel,
    String time,
    String endTime
  ) {
    if (time == null) {
      throw new BadRequestException(
        DataCoreUiCode.ErrorCode.REQUEST_MESSAGE_PARSING_ERROR,
        "invalid timerel params"
      );
    }

    if (
      timerel.equalsIgnoreCase(
        DataCoreUiCode.TemporalOperator.BETWEEN_REL.getCode()
      ) &&
      endTime == null
    ) {
      throw new BadRequestException(
