...
import io.dtonic.dhubingestmodule.common.code.DataCoreUiCode.Operation;
...
public class CommonParamUtil {
   ...
   * @return
   */
  public static String extractEntityTypeById(String entityId)
    ...
    if (splitId == null || splitId.length <= RESOURCE_ID_ENTITY_TYPE_INDEX) {
      throw new BadRequestException(
        DataCoreUiCode.ErrorCode.INVALID_ENTITY_TYPE,
        "Not found entityType. entityId=" + entityId
      );
    }
   ...
   * @return
   */
  public static String extractEntityTypeByContentType(String contentType)
    throws BadRequestException {
    if (ValidateUtil.isEmptyData(contentType)) return null;

    String[] splitType = contentType.split(CONTENT_TYPE_SEPARATOR);
    if (
      splitType == null || splitType.length <= CONTENT_TYPE_ENTITY_TYPE_INDEX
    ) {
      throw new BadRequestException(
        DataCoreUiCode.ErrorCode.INVALID_ENTITY_TYPE,
        "Not found entityType. contentType=" + contentType
      );
    }
